        ...
        if submit:
            if user and pw:
                ...
                if not st.session_state['chat_history']:
                    st.session_state['chat_history'].append({"role": "bot","message":f"Halo {user}! Aku AiLen. Isi profilmu di sidebar, lalu tanya apa saja."})
...
def draw_gauge(bmi):
    ...
    fig=go.Figure(go.Indicator(mode="gauge+number",value=bmi,title={"text":"BMI Score"},gauge={"axis":{"range":[10,45]},"bar":{"color":color}}))
